...
- Inside `codex:codex-agent`, do not turn a failed or incomplete Codex run into a Claude-side implementation attempt. Return the failure to the orchestrating Claude agent, which decides the next step.
- For `codex:codex-agent`, if Codex was never successfully invoked, do not generate a substitute answer at all.
